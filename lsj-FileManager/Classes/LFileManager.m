...
//  lsj-FileManager
//
//  Created by Lishangjing on 2024/3/16.
//
...
+ (NSString *)applicationSupportDirectory {
    if (![LFileManager isExistsAtPath:[NSSearchPathForDirectoriesInDomains(NSApplicationSupportDirectory, NSUserDomainMask, YES) firstObject]]){
        BOOL success = [LFileManager createDirectoryAtPath:[NSSearchPathForDirectoriesInDomains(NSApplicationSupportDirectory, NSUserDomainMask, YES) firstObject]];
        if (success) {
            NSLog(@"创建 NSApplicationSupportDirectory 目录: 成功");
        }else{
            NSLog(@"创建 NSApplicationSupportDirectory 目录: 失败");
            return nil;
        }
    }
