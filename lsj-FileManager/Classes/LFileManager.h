...
/// 当我们将 Document 共享给用户看时（File App），applicationSupportDirectory 可以成为我们第二个存放点，这个目录是不会被用户所发现的
/// 默认情况下该目录是不存在的，在调用时，会自动创建该目录
...
/**
 文件遍历
 @param path 目录的绝对路径
 @param deep 是否深遍历 (1. 浅遍历：返回当前目录下的所有文件和文件夹； 2. 深遍历：返回当前目录下及子目录下的所有文件和文件夹)
 @return 遍历结果数组
 */
